...
export default function DiscordSendButton({ channelId, payload, label = 'Wyślij na Discord' }: DiscordSendButtonProps) {
  ...
  const send = useCallback(async () => {
    ...
    try {
      const res = await fetch('/api/discord/send-roast', {
        ...
        body: JSON.stringify({
          channelId,
          pin: localStorage.getItem('podtekst-discord-pin') ?? '',
          ...payload,
        }),
      });
      if (!res.ok) {
